...
| **Frozen Cells** | Stall a specific line from completing. | Requires `03` Section 3.2's reserved `FROZEN` state. | Would be visually distinct from `FILLED` (future Art doc). | Moderate–high, forces rerouting. | Blocked on the same cross-document activation as Hard Pre-fill. | **Later.** |
| **Blocked Cells** | Deny placement into specific cells temporarily. | Requires `03` Section 3.2's reserved `BLOCKED` state. | Visually distinct, non-interactable cell marker. | Moderate. | Same activation dependency. | **Later.** |
...
|---|---|---|---|
| "Ice Mage" | *Clear 3 rows before the third freeze cycle.* | **Row Count** Primary + **Conditional Phase**-style Enemy-action counter, dependent on the `FROZEN` state / freeze-cycle mechanic. | **No** — depends on `03` Section 3.2's `FROZEN` state and a future Enemy Content doc's "freeze cycle" mechanic; documented here as a target design, not a shippable Absolute MVP encounter. |
...
|---|---|---|---|
| **Ice Mage** | Freeze-cycle pressure (`FROZEN` state) | No (single mechanic per Section 21) | **No** — blocked on `03` Section 3.2 activation, same as Section 9. Documented as a target design for MVP+. |
