...
- **Primary Objective:** exactly one per encounter (Section 3); its `success_condition` ends the Battle in victory; any category-specific `failure_condition` (e.g. Move Limit exceeded) or Board Lock causes a **Battle Failure** and subsequent **Run Defeat** (Gameplay Spec Section 15).
...
| **Frozen Cells** | Stall a specific line from completing. | Uses `03` Section 3.2's `FROZEN` modifier. | Visually distinct from normal cells (future Art doc). | Moderate–high, forces rerouting. | Low — `FROZEN` logic is now MVP-active via `03` Section 9's predicates. | **MVP.** |
| **Blocked Cells** | Deny placement into specific cells temporarily. | Uses `03` Section 3.2's `BLOCKED` modifier. | Visually distinct, non-interactable cell marker. | Moderate. | Low — logic is MVP-active. | **MVP.** |
...
|---|---|---|---|
| "Ice Mage" | *Clear 3 rows before the third freeze cycle.* | **Row Count** Primary + **Conditional Phase**-style Enemy-action counter, dependent on the `FROZEN` modifier / freeze-cycle mechanic. | **Yes** — `FROZEN` is now MVP-active. |
...
|---|---|---|---|
| **Ice Mage** | Freeze-cycle pressure (`FROZEN` modifier) | No (single mechanic per Section 21) | **Yes.** |
