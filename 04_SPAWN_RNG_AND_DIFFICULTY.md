...
- **Autosave compatibility:** the Run Seed and current draw index (referred to as `rngStateCalls` in the save schema) are part of the autosaved Run state (`24` Section 2), so a resumed Run continues the identical RNG stream by advancing the PRNG to the exact call count rather than generating a fresh random seed.
